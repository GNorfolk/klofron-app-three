  ...
  return (
    <main>
      ...
      { familyPeopleBetrothals.length > 0 ? familyPeopleBetrothals.map(({ person_id, person_name, person_betrothal_receipts }) => (
        <a href={"/person/" + person_id + "/proposal"} className="p-6 pt-2 pb-2">
          <div className="flex">
            <div className="flex items-center text-sm text-gray-500 dark:text-gray-400">
              <ChurchIcon className="w-5 h-5 min-w-5 min-h-5 mx-2" />
              <span className='whitespace-nowrap'>{person_name} has {person_betrothal_receipts.length} betrothal.</span>
            </div>
          </div>
        </a>
      )) :
