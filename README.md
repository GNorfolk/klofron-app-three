...
```bash
mysqldump -h react-app.casjyk0nx1x8.eu-west-1.rds.amazonaws.com -u root -p ka3 > dump-2024-08-02.sql
sed 's/\sDEFINER=`[^`]*`@`[^`]*`//g' -i dump-2024-08-02.sql
sed -i 's/SET @MYSQLDUMP_TEMP_LOG_BIN/-- SET @MYSQLDUMP_TEMP_LOG_BIN/g' dump-2024-08-02.sql
sed -i 's/SET @@SESSION.SQL_LOG_BIN/-- SET @@SESSION.SQL_LOG_BIN/g' dump-2024-08-02.sql
sed -i 's/SET @@GLOBAL.GTID_PURGED/-- SET @@GLOBAL.GTID_PURGED/g' dump-2024-08-02.sql
sed -i 's/SET @@SESSION.SQL_LOG_BIN/-- SET @@SESSION.SQL_LOG_BIN/g' dump-2024-08-02.sql
sed -i 's/utf8mb4_0900_ai_ci/utf8mb4_unicode_ci/g' dump-2024-08-02.sql
aws s3 cp dump-2024-08-02.sql s3://ka3-db-dumps
aws s3 cp s3://ka3-db-dumps/dump-2024-08-02.sql .
mysql -u root -p ka3 < dump-2024-08-02.sql
```
...
# MySQL
```sql
ALTER TABLE action ADD `experience_multiplier` INT NOT NULL DEFAULT 1;
```
